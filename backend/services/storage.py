...
def delete_rule(factory_id: str, rule_id: str) -> bool:
    factory = get_factory(factory_id)
    if not factory:
        return False
    before = len(factory.rules)
    factory.rules = [r for r in factory.rules if r.id != rule_id]
    save_factory(factory)
    return len(factory.rules) < before


def upsert_rule(factory_id: str, rule: Rule) -> bool:
    factory = get_factory(factory_id)
    if not factory:
        return False
    for i, r in enumerate(factory.rules):
        if r.id == rule.id:
            factory.rules[i] = rule
            save_factory(factory)
            return True
    factory.rules.append(rule)
    save_factory(factory)
    return True


def get_rule(factory_id: str, rule_id: str) -> Optional[Rule]:
    factory = get_factory(factory_id)
    if not factory:
        return None
    return next((r for r in factory.rules if r.id == rule_id), None)
